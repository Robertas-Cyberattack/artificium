...
from projects.models import ClientProject, ProjectFile, ProjectMessage
...
def create_checkout_session(request):
    ...
    if not settings.STRIPE_SECRET_KEY:
        messages.error(
            request,
            "Stripe is not configured yet. Add your Stripe keys first.",
        )
        return redirect("pay_quote")
...
def download_invoice(request, project_id):
    ...
    invoice.amount = project.price
    invoice.description = f"Payment for {project.title}"
    invoice.status = "paid" if project.is_paid else "unpaid"
    invoice.save()
...
def delete_project_file(request, file_id):
    ...
    if not request.user.is_staff and file_obj.project.client != request.user:
        return redirect("dashboard")
...
def dashboard(request):
    ...
    projects = ClientProject.objects.filter(client=request.user).order_by("-created_at")
...
def client_edit_project(request, project_id):
    ...
    if project.is_paid or project.status not in ["requested"]:
        messages.error(
            request,
            "You can only edit projects before they are quoted or paid.",
        )
        return redirect("dashboard")
...
def client_delete_project(request, project_id):
    ...
    if project.is_paid or project.status not in ["requested"]:
        messages.error(
            request,
            "You can only delete projects before they are quoted or paid.",
        )
        return redirect("dashboard")
    ...
    return render(
        request,
        "home/client_delete_project.html",
        {
            "project": project,
        },
    )
...
def client_project_messages(request, project_id):
    ...
    return render(
        request,
        "home/client_project_messages.html",
        {
            "project": project,
...
def admin_project_messages(request, project_id):
    ...
    return render(
        request,
        "home/admin_project_messages.html",
        {
            "project": project,
            "project_messages": project_messages,
        },
...
def client_delete_message(request, message_id):
    ...
    return render(
        request,
        "home/client_delete_message.html",
        {
            "message_obj": message_obj,
        },
    )
...
def admin_edit_message(request, message_id):
    ...
    if request.method == "POST":
        ...
        if message_obj.is_admin_message:
            message_obj.is_read_by_client = False
        else:
            message_obj.is_read_by_admin = True

        message_obj.save()
        messages.success(request, "Message updated successfully.")

        return redirect(
            "admin_project_messages",
            project_id=message_obj.project.id,
        )

    return render(
        request,
        "home/admin_edit_message.html",
        {
            "message_obj": message_obj,
        },
    )
...
def admin_delete_message(request, message_id):
    ...
    return render(
        request,
        "home/admin_delete_message.html",
        {
            "message_obj": message_obj,
        },
    )
