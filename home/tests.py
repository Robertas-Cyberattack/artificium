...
from projects.models import ClientProject, ProjectFile
...
class ArtificiumTests(TestCase):
    ...
    def setUp(self):
        self.client_user = User.objects.create_user(
            username="clientuser", email="client@test.com", password="testpass123"
        )

        self.other_client = User.objects.create_user(
            username="otherclient", email="other@test.com", password="testpass123"
        )
    ...
    def test_invoice_download_creates_invoice(self):
        ...
        response = self.client.get(reverse("download_invoice", args=[self.project.id]))

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response["Content-Type"], "application/pdf")
        self.assertTrue(Invoice.objects.filter(project=self.project).exists())
    ...
    def test_client_can_upload_project_file(self):
        ...
        response = self.client.post(
            ...
            {"title": "Test Upload", "file": test_file},
        )

        self.assertEqual(response.status_code, 302)
        self.assertTrue(
            ProjectFile.objects.filter(
                project=self.project, title="Test Upload"
            ).exists()
        )
    ...
    def test_client_can_delete_project_file(self):
        project_file = ProjectFile.objects.create(
            ...
            file=SimpleUploadedFile(
                "delete_me.txt", b"Delete this file.", content_type="text/plain"
            ),
        ...
        self.assertEqual(response.status_code, 302)
        self.assertFalse(ProjectFile.objects.filter(id=project_file.id).exists())
